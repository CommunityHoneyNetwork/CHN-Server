...
def deploy_mgmt():
    ...
    return render_template(
            'ui/script.html', scripts=Script.query.order_by(
                Script.date.desc()),
            script=script)
...
def get_credentials_payloads(clio):
    ...
    credentials_payloads += clio.hpfeed.get_payloads(
        {'limit': 10000}, {"channel": "kippo.sessions"})[2]
    credentials_payloads += clio.hpfeed.get_payloads(
        {'limit': 10000}, {"channel": "cowrie.sessions"})[2]
...
def graph_passwords():
    ...
    for password in top_passwords:
        bar_chart.add(password[0], [
            {'label': str(password[0]), 'xlink': '', 'value':password[1]}])
...
def graph_users():
    ...
    for user in top_users:
        bar_chart.add(user[0], [{'label': str(user[0]), 'xlink':'',
                                 'value': user[1]}])
...
def graph_combos():
    ...
    for combo in top_combos:
        bar_chart.add(combo[0], [{'label': str(combo[0]), 'xlink':'',
                                 'value':combo[1]}])
...
def top_kippo_cowrie_attackers(clio):
    ...
    return [
        {'source_ip': ip, 'count': count} for ip, count in sorted(
