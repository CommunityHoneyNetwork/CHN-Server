...
def new():
    ...
    if "ftpdownload" in g_dionaea.\
            config()['modules']['python']['ihandlers']['handlers']:
        import dionaea.ftp
        g_handlers.append(dionaea.ftp.ftpdownloadhandler(
            'dionaea.download.offer'))

    if "tftpdownload" in g_dionaea.\
            config()['modules']['python']['ihandlers']['handlers']:
        g_handlers.append(dionaea.tftp.tftpdownloadhandler(
            'dionaea.download.offer'))

    if "emuprofile" in g_dionaea.\
            config()['modules']['python']['ihandlers']['handlers']:
        g_handlers.append(
            dionaea.emu.emuprofilehandler('dionaea.module.emu.profile'))

    if "cmdshell" in g_dionaea.\
            config()['modules']['python']['ihandlers']['handlers']:
        g_handlers.append(
            dionaea.cmd.cmdshellhandler('dionaea.service.shell.*'))

    if "store" in g_dionaea.\
            config()['modules']['python']['ihandlers']['handlers']:
        g_handlers.append(dionaea.store.storehandler(
            'dionaea.download.complete'))

    if "uniquedownload" in g_dionaea.\
            config()['modules']['python']['ihandlers']['handlers']:
        g_handlers.append(dionaea.test.uniquedownloadihandler(
            'dionaea.download.complete.unique'))

    if "surfids" in g_dionaea.\
            config()['modules']['python']['ihandlers']['handlers']:
        import dionaea.surfids
        g_handlers.append(dionaea.surfids.surfidshandler('*'))

    if "logsql" in g_dionaea.\
            config()['modules']['python']['ihandlers']['handlers']:
    ...
    if "p0f" in g_dionaea.\
            config()['modules']['python']['ihandlers']['handlers']:
        import dionaea.p0f
        g_handlers.append(dionaea.p0f.p0fhandler(
            g_dionaea.config()['modules']['python']['p0f']['path']))

    if "logxmpp" in g_dionaea.\
            config()['modules']['python']['ihandlers']['handlers']:
        ...
        for client in g_dionaea.config()['modules']['python']['logxmpp']:
            ...
            print(
                ("client %s \n\tserver %s:%s username %s password %s resource"
                 "%s muc %s\n\t%s" % (
                    client, conf['server'], conf['port'], conf['username'],
                    conf['password'], resource, conf['muc'], conf['config'])))
            x = dionaea.logxmpp.logxmpp(
                conf['server'], int(conf['port']), conf['username'],
                conf['password'], resource, conf['muc'], conf['config'])
            g_handlers.append(x)
